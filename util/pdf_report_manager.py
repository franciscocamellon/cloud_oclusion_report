...
class generatePDF(list):
    def __init__(self, list) -> None:
        self.om_name = self.transformStringToHTMLtext(self.convertCGEOname(list[0]))
        self.doc_number = self.transformStringToHTMLtext(list[1])
        self.block = self.transformStringToHTMLtext(list[2])
        self.work_month = self.transformStringToHTMLtext(list[3])
        self.work_year = self.transformStringToHTMLtext(list[4])
        self.finish_project_date = self.transformStringToHTMLtext(list[5])
        self.conformity = self.transformStringToHTMLtext(list[6])
        self.evaluation_result = self.transformStringToHTMLtext(list[7])
        self.evaluation_person = self.transformStringToHTMLtext(list[8])
        self.manage_person = self.transformStringToHTMLtext(list[9])
        self.map_scale = '1:10.000'
        self.src = 'SIRGAS 2000 UTM 24S'
        self.product_type = 'Ortoimagem'
        self.doc_date = self.transformStringToHTMLtext(list[10])
        self.localAndTime = self.localDocument(list[0]) + self.doc_date
    ...
    def transformStringToHTMLtext(self, string):
        ...
        # Começa na posição 3 e termina em -1, pois a função .escape e .encode transformam no formato: b'TEXTO CONVERTIDO'
